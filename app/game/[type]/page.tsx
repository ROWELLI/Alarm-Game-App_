...
import { detectCcc, detectDbdbd, detectRps } from "@/lib/api"

type GameResult = "WIN" | "LOSE" | "DRAW" | null
type GameChoice = "rock" | "paper" | "scissor" | "left" | "right" | "front" | null
...
export default function GamePage({ params }: { params: Promise<{ type: string }> }) {
  ...
  const stopAlarmSound = () => {
    const win = window as typeof window & { alarmAudio?: HTMLAudioElement }
    if (win.alarmAudio) {
      win.alarmAudio.pause()
      win.alarmAudio.currentTime = 0
    }
  }
    ...
    const startCamera = async () => {
      try {
        if (
          ...
          console.error("이 환경에서는 카메라 API를 사용할 수 없습니다.")
  ...
  const decideResult = (user: GameChoice, computer: GameChoice): GameResult => {
    if (!user || !computer) return "LOSE"

    if (type === "chamchamcham") {
      return user !== computer ? "WIN" : "LOSE"
    }
  ...
  const detectPresence = async (blob: Blob) => {
    ...
    if (type === "chamchamcham") {
      const detected = await detectCcc(blob)
      return detected.face_detected
    }

    return false
  }

  const detectFinalChoice = async (blob: Blob): Promise<GameChoice> => {
    if (type === "rps") {
      const detected = await detectRps(blob)
    ...
    if (type === "chamchamcham") {
      ...
      if (!detected.face_detected) return null
      
      const conf = detected.confidence ?? 0
      if ((detected.label === "left" || detected.label === "right") && conf < 0.55) {
        return "front"
      }

      return label === "left" || label === "right" || label === "front"
        ? label
        : null
    }
  ...
  useEffect(() => {
    ...
    if (type !== "rps" && type !== "dibidibidip" && type !== "chamchamcham") return
    ...
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="flex justify-between text-sm text-muted-foreground mb-4">
          ...
          {userChoice && <span>USER: {choiceLabels[userChoice]}</span>}
          {computerChoice && <span>COM: {choiceLabels[computerChoice]}</span>}
        </div>

        <div className="relative h-[420px] bg-muted rounded-lg overflow-hidden border-2 border-[#3B9AE8] mb-4">
          <video
            ref={videoRef}
            autoPlay
            playsInline
        ...
        <div className="h-[120px] bg-background border-2 border-[#3B9AE8] rounded-lg flex items-center justify-center">
...
function ComputerGesture({ choice, type }: { choice: GameChoice; type: string }) {
  if (type === "chamchamcham") {
    return (
      <div className="text-8xl font-bold">
        {choice === "left" ? "←" : choice === "right" ? "→" : choice === "front" ? "↑" : "?"}
      </div>
    )
  }
